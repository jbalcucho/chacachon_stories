...
Para soportar la paginación, la consistencia de los personajes y la inserción dinámica de los nombres, diseñaremos un modelo relacional en **Neon Postgres + Prisma** (mismo patrón Balcu que rotatudisfraz) dividido en tres grandes bloques: El Catálogo (Core), La Localización (Textos/Audios) y La Sesión del Usuario. Auth: **NextAuth + Google**. Ver [StackTecnico.md](./StackTecnico.md).
...
id (UUID, Primary Key): ID único del usuario en Prisma (`User`), vinculado a la sesión NextAuth.
...
Por debajo, Prisma/Postgres ejecuta un JOIN eficiente que trae la estructura de la página y el texto exacto que hace match con el acento.
...
El schema canónico de Sprint 0+ vive en Prisma (`prisma/schema.prisma`) sobre Neon. No uses el SQL legacy de Supabase Auth (`auth.users`).

Para el catálogo y perfiles actuales:

- Modelos: `User`, `FamilyProfile` (JSONB), `Story` — ver [docs/database.md](./docs/database.md)
- Auth: NextAuth + Google — ver [StackTecnico.md](./StackTecnico.md)
- Interpolación: `src/lib/interpolation.ts` + perfil en `/familia`

Tablas futuras del reader paginado (`pages`, `localized_texts`, etc.) se añadirán por migraciones Prisma cuando el reader deje de ser HTML estático.
...
npm run setup:neon   # o db:migrate + db:seed
npm run dev
```

Con esa base, el terreno está preparado para inyectar variables de texto (como `{{niño_1}}`) y personalizar cuentos desde el perfil familiar.